...
public class CrawlerThread implements Runnable
{
	...
	public void run()
	{
		...
		while (true)
		{
			...
			{
				System.out
						.println("Writing url [" + url + "] to worker " + idx);
				WriteToFile.write(url, idx);
				continue;
			}

			/**
			 * Check if document already exists in db. Then send head to check
			 * if modified
			 */
			...
			if (RobotsInfoDA.contains(domain))
			{
				...
				if (modified)
				{
					...
					{
						System.out
								.println("Error fetching Robots.txt modified: "
										+ e);
					}
					urlQueue.enqueue(url);
					// DBWrapper.closeDBWrapper();
				...
				{
					if (info.getRobotsInfo().getAllowedLinks(agent_match)
							.contains(filepath))
					{
						allowed = true;
					}
				...
				// If robots.txt has not been modified, check for
				// allowed/disallowed links
				boolean disallowed = false;
				if (info.getRobotsInfo().getDisallowedLinks(agent_match) != null)
				{
					for (String link : info.getRobotsInfo().getDisallowedLinks(
							agent_match))
					{
			...
			DocumentRecord docrecord = new DocumentRecord(url, document,
					current.getTime());
