...
public class XPathParser {
	 ...
	 */
	private static enum symbol {ForwardSlash, OpenParen, CloseParen, OpenBracket, CloseBracket, Equals, At, DoubleQuote, Text, Contains, Comma};
	private static EnumMap<symbol, String> symbolMap = new EnumMap<symbol, String>(symbol.class);
	...
	public static void parseXPath(XPath xPath)
	{
		...
		if(rootStep == null || rootStep.getAfterStep()!=xPath.getTokens().size())
		{
...
/*				else
	...
	private static XPathStep step(ArrayList<String> tokens, int next)
	{
		...
		if(tokens.size()<=next || tokens.get(next).contains("::") || !tokens.get(next).matches("[a-zA-Z0-9\\-]*"))
		{
			return step;
		}
		//check if the first token is a nodename - check if the first token is not a special symbol
		step = new XPathStep();
		...
		{
			next = step.getTests().get(step.getTests().size()-1).getAfterTest()+1;
		}
		...
		/*if(next<=beforeStep+1 && next<tokens.size())
		{
			next=beforeStep;
		}*/
	...
	private static ArrayList<XPathTest> recursiveTest(ArrayList<String> tokens, int next)
	{
		...
		if(matchToken(tokens.get(next), symbol.OpenBracket))
		{
			...
			else
			{
				...
				if(matchToken(tokens.get(test.getAfterTest()), symbol.CloseBracket))
				{
					//System.out.println("]");
					afterAllTests = test.getAfterTest()+1;
					ArrayList<XPathTest> otherTests = recursiveTest(tokens, afterAllTests);
					if(otherTests!=null)
					{
			...
			if(afterTest>(next+1))
			{
				if(tokens.size()<=afterTest)
				{
					return next;
				}
				if(matchToken(tokens.get(afterTest), symbol.CloseBracket))
				{
					//System.out.println("]");
					afterAllTests = afterTest+1;
					afterAllTests = recursiveTest(tokens, afterAllTests);
				}
			}*/
		}
		return tests;
	...
	private static XPathTest test(ArrayList<String>tokens, int next)
	{
		...
		//test can be a step or a [text()=".."] or a [contains(text(),".."] or [@attname = ".."]
		if(matchToken(tokens.get(next), symbol.Text) && tokens.size()>=next+7)
		{
			test = new XPathTest();
			test.setTextFilter(true);
			next++;
			if(matchToken(tokens.get(next++), symbol.OpenParen) && matchToken(tokens.get(next++), symbol.CloseParen) && matchToken(tokens.get(next++), symbol.Equals) && matchToken(tokens.get(next++), symbol.DoubleQuote)) 
			{
		...
		else if(matchToken(tokens.get(next), symbol.Contains) && tokens.size()>=next+10)
		{
			test = new XPathTest();
			test.setContainsFilter(true);
			next++;
			if(matchToken(tokens.get(next++), symbol.OpenParen) && matchToken(tokens.get(next++), symbol.Text) && matchToken(tokens.get(next++), symbol.OpenParen) && matchToken(tokens.get(next++), symbol.CloseParen) && matchToken(tokens.get(next++), symbol.Comma) && matchToken(tokens.get(next++), symbol.DoubleQuote))
			{
				test.setQueryString(tokens.get(next++));
				if(!matchToken(tokens.get(next++), symbol.DoubleQuote))
				{
		...
		else
		{
			...
			//System.out.println("In Test after step matching - "+tokens.get(next)+" "+matchToken(tokens.get(next), symbol.Text));
		}
		//System.out.println("Test - "+test);
		return test;
	}
